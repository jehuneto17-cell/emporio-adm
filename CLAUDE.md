...
| 2026-08-21 | `firestore.js` — `getPedidos`/`getPedidosArquivados`: removido `orderBy('createdAt', 'desc')` da query do Firestore. Esse orderBy omite silenciosamente (sem erro) qualquer pedido cujo documento não tenha o campo `createdAt` (ex: pedido criado manualmente/fora do fluxo padrão do app) — o badge de "novos pedidos" da sidebar (que usa `onSnapshot`/`where` sem orderBy) contava 1 pedido, mas a tela de Pedidos mostrava lista vazia. Agora busca até 200 docs sem ordenação no servidor e ordena em memória por `createdAt` (fallback 0 quando ausente), preservando o limite de 50 resultados retornados. |
