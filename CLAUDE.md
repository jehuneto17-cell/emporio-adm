...
| 2026-07-22 | dashboard-app.jsx: corrigida inconsistência entre buildChartData (comparação de string exata, o.date === key) e computeDashMetrics (comparação de datas reais via parseDateBR) — pedidos com data salva sem zero à esquerda (ex: "5/7/2026") eram contados nos cards do topo mas não apareciam nas barras do gráfico. Agora ambas as funções usam a mesma lógica de comparação de datas real. |
| 2026-07-22 | pedidos-app.jsx e firestore.js: implementada a visualização real de pedidos arquivados (antes a aba "Arquivados" sempre retornava vazio) e adicionado botão de desarquivar (a função DB.desarquivarPedido já existia mas nunca era usada na interface). Nova função DB.getPedidosArquivados() busca especificamente os pedidos com arquivado: true. |
...
| 2026-08-17 | Duplicar produto implementado. `app.jsx`: botão "Duplicar" (`IconCopy`, já existente em `icons.jsx`) adicionado entre o lápis e o olho na coluna AÇÕES; `onAction('duplicate')` navega para `Editar Produto.html?copyOf=ID`; largura da coluna Ações de 130 → 160 para caber o 4º ícone. `edit-app.jsx`: `useEffect` de carregamento passa a aceitar `?copyOf=` além de `?id=` — em modo cópia `productId` fica `null` (fazendo `handleSave` cair em `DB.addProduto` sem alterar essa função), SKU e `updatedAt` são zerados, estoque vai a 0 e os demais campos (incluindo `images[]`) são copiados; novo estado `copySourceName` alimenta faixa de aviso dourada acima das colunas ("Você está criando uma cópia de X. O produto original não será alterado."). Nada é gravado no Firestore até o admin clicar em Salvar. `firestore.js`: `getProdutos` passa a mapear `varType` (era salvo mas nunca recarregado — o tipo de variação sempre voltava ao default 'peso' ao editar). |
| 2026-08-21 | `estoque-app.jsx`: corrigido filtro de categoria que nunca funcionava — `produtoToStockItem` guarda `cat` como o ID da categoria (`p.category`), mas o dropdown listava e comparava pelo nome; agora um mapa `catNameById` (montado a partir de `DB.getCategorias()`) resolve o nome tanto para exibir na tabela/CSV quanto para o filtro comparar corretamente. Também adicionada ordenação alfabética (`localeCompare pt-BR`) da lista de estoque, aplicada tanto no carregamento inicial quanto após qualquer filtro. `STOCK_CATEGORIES` (array hardcoded morto, não usado desde a conexão com Firestore) removido. |
| 2026-08-21 | `estoque-app.jsx`: corrigido filtro que retornava 0 produtos ao selecionar uma subcategoria — `produtoToStockItem` só copiava `p.category` (categoria pai), nunca `p.subcategory`, então subcategorias nunca batiam com nenhum item. Adicionado campo `subcat` ao item de estoque; filtro agora casa o valor selecionado com a categoria pai OU a subcategoria do produto. Coluna CATEGORIA e CSV passam a exibir a subcategoria (mais específica) quando o produto tiver uma, com fallback para a categoria principal. |
| 2026-08-21 | `firestore.js` — `getPedidos`/`getPedidosArquivados`: removido `orderBy('createdAt', 'desc')` da query do Firestore. Esse orderBy omite silenciosamente (sem erro) qualquer pedido cujo documento não tenha o campo `createdAt` (ex: pedido criado manualmente/fora do fluxo padrão do app). Agora busca até 200 docs sem ordenação no servidor e ordena em memória por `createdAt` (fallback 0 quando ausente), preservando o limite de 50 resultados retornados. |
| 2026-08-21 | Investigado (via inspeção direta do Firestore pelo console do browser) por que a aba "Aguardando pagamento" ficava vazia mesmo com o badge da sidebar mostrando "1": o único pedido com esse status tinha `arquivado: true` — corretamente excluído de `getPedidos()`/"Todos". Dois bugs reais encontrados e corrigidos: (1) `pedidos-app.jsx` — `DB.getPedidosArquivados()` só era chamado quando o admin clicava na aba "Arquivados" (`useEffect` com guarda `if (tab !== 'Arquivados') return`), então o contador da aba (`archivedOrders.length`) sempre mostrava 0 até o primeiro clique; passou a carregar no mount, igual `getPedidos()`. (2) `shared.jsx` — o listener `onSnapshot` do badge da sidebar filtrava só por `status === 'Aguardando pagamento'`, sem checar `arquivado`, contando pedidos arquivados que não deveriam gerar notificação; adicionado filtro `!d.data().arquivado` no callback do snapshot. |
| 2026-08-21 | `pedidos-app.jsx`: corrigido empty state da aba "Arquivados" — mesmo com o contador mostrando corretamente "35" e a paginação calculando "1–20 de 35", a tabela exibia "Nenhum pedido ainda" porque a condição de loading/empty state (`loading`/`orders.length === 0`) só considerava a lista de pedidos não arquivados (`orders`), nunca `archivedOrders`/`loadingArchived`, interceptando a renderização antes de chegar em `filtered`/`paginated`. Ambas as condições agora ramificam por `tab === 'Arquivados'`. |
| 2026-08-21 | Excluir pedido permanentemente implementado — disponível apenas na aba Arquivados (drawer só mostra o botão quando `isArchived`, ao lado de "Desarquivar"), para evitar exclusão acidental de pedido ativo. `firestore.js` ganhou `DB.deletePedido(id)` (delete direto do doc, mesmo padrão de `deleteCupom`/`deleteCategoria`). `pedidos-app.jsx`: `deleteOrder(id)` usa o `ConfirmModal` já existente ("Excluir este pedido permanentemente? Essa ação não pode ser desfeita."), remove de `archivedOrders` e fecha o drawer ao confirmar. `OrderDrawer` ganhou prop `onDelete`. |
| 2026-08-21 | Diagnosticado (via teste direto no console do browser) por que "Excluir" não funcionava mesmo com o modal de confirmação aparecendo normalmente: as regras do Firestore publicadas no Console tinham `allow delete: if false;` no bloco `match /pedidos/{pedidoId}`, bloqueando exclusão para todo mundo, inclusive admin — decisão de segurança anterior para proteger histórico de pedidos. `FIRESTORE_RULES.md` do repositório estava desatualizado (mostrava uma regra genérica de admin com acesso total que não refletia o que estava de fato publicado) e por isso não ajudou a prever o bloqueio. Regra publicada corrigida manualmente no Console para `allow delete: if isAdmin();`; `FIRESTORE_RULES.md` reescrito para refletir fielmente as regras reais (incluindo `statusPermitidoCliente()`, `users/{uid}/orders`, e a assimetria documentada de que o espelho `/pedidos` permite delete do admin mas `users/{uid}/orders` não). |
| 2026-08-21 | `styles.css`: corrigido drawer lateral (`.drawer`/`.drawer-overlay`, usado em `pedidos-app.jsx` e `clientes-app.jsx`) que "rolava junto" com a página de fundo — `position: absolute` ancorava o drawer no `.stage` (que cresce com o conteúdo da tabela, bem mais alto que a viewport), então ao rolar a lista para baixo o cabeçalho/conteúdo do drawer saía de vista em vez de ficar fixo na tela como um painel lateral deveria. Trocado para `position: fixed` em ambas as classes, ancorando à viewport independente do scroll da página. |
| 2026-08-31 | `app.jsx`: corrigido filtro "Esgotado" da página de Produtos, que sempre retornava 0 resultados mesmo com o card do topo mostrando corretamente "32 produtos esgotados" — o filtro comparava `r.status === 'Esgotado'`, mas produto nunca tem esse valor salvo no campo `status` (schema real é Ativo/Inativo; "esgotado" é derivado de `stock === 0`, mesma regra já usada pelos cards e pelo aviso amarelo da página). Filtro agora usa `stock === 0` quando a opção selecionada é "Esgotado". `StatusBadge` da tabela também corrigida para exibir "Esgotado" quando `p.stock === 0`, independente do campo `status`. |
| 2026-08-31 | `app.jsx`: corrigido card "produtos com estoque baixo" e coluna Estoque da tabela, que ignoravam o `minStock` cadastrado por produto e usavam limiar fixo hardcoded (`stock <= 10` no card, `stock <= 8` na célula) — um produto com estoque mínimo = 1un e 5un em estoque aparecia como "baixo" mesmo estando 5x acima do próprio mínimo. Ambos agora comparam `stock <= (minStock ?? 5)`, mesmo fallback já usado no CSV e em `firestore.js`. `estoque-app.jsx` tinha o mesmo bug — corrigido no mesmo dia (ver linha abaixo). |
| 2026-08-31 | `estoque-app.jsx`: `produtoToStockItem` tinha `min: 5` hardcoded para todo produto, ignorando o `minStock` cadastrado — status "Baixo"/"Normal", cards do topo e barra de progresso usam `s.min` (`statusOf`), então nenhum deles respeitava o mínimo real. Corrigido para `min: p.minStock != null ? p.minStock : 5`. |
| 2026-08-31 | `estoque-app.jsx`: tabela de Controle de estoque passou a exibir a foto real do produto (`images[0]` ou `imageUrl`, mesma prioridade usada em `app.jsx`) em vez do círculo de iniciais fixo; fallback para iniciais mantido quando o produto não tem foto cadastrada. `produtoToStockItem` ganhou campo `photo`. |
| 2026-09-01 | `app.jsx`: corrigida página de Produtos que ficava presa numa página de paginação inexistente (ex: "29–29 de 29 produtos" e tabela vazia com "Nenhum produto cadastrado", mesmo com os cards do topo mostrando 260/15/40 corretamente) — `page` nunca era corrigido quando `filtered` encolhia (troca de filtro, ou carregamento assíncrono do Firestore com menos itens que a sessão anterior), então o slice da tabela ficava além do fim do array. Adicionado `safePage = Math.min(page, totalPages)`, usado no slice e na exibição "X–Y de Z"; `Pagination` recebe `safePage` em vez de `page`. |
| 2026-09-01 | Mesmo bug de paginação (página presa em branco após filtro reduzir o total) corrigido em `pedidos-app.jsx`, `clientes-app.jsx`, `cupons-app.jsx` e `estoque-app.jsx` — todas usavam `page` direto no `.slice()`/exibição/botões sem checar contra `totalPages`. Mesmo padrão `safePage = Math.min(page, totalPages)` aplicado nas 4. Pendente (não corrigido): `order-detail-app.jsx` lê `order.timeline` para o card "Histórico de status", mas nenhum lugar do código grava esse campo no Firestore — card sempre vazio para todo pedido. |
| 2026-09-01 | `app.jsx`: larguras das colunas da tabela de Produtos ajustadas (`colgroup` de `ProductsTable`) — coluna "Produto" antes era flexível (`<col />`, crescia sem limite e espremia o resto), fixada em 260px; Categoria 150→190, Status 130→150, Ações 160→170, para acomodar melhor o badge de categoria (que quebrava em várias linhas) em telas largas. Só CSS de largura, sem mudança de lógica. |
